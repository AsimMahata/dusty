...
export const MediaDetailListChild: React.FC<MediaDetailListChildProps> = ({
    ...
    if (mediaType === 'video') {
        extraActions.push({ separator: true, label: '', onClick: () => { } });
        extraActions.push({ label: 'Mark as Watched', icon: CHECK_CIRCLE_2_ICON_16, color: COLORS.MEDIA.WATCHED, onClick: () => onUpdateStatus?.(child.id, 'watched') });
        extraActions.push({ label: 'Mark as Unwatched', icon: EYE_OFF_ICON_16, color: COLORS.MEDIA.UNWATCHED, onClick: () => onUpdateStatus?.(child.id, 'unwatched') });
        extraActions.push({ label: 'Mark as Default', icon: CIRCLE_ICON_16, color: 'var(--text-muted)', onClick: () => onUpdateStatus?.(child.id, 'default') });
    }

    const actions: ActionItem[] = child.path
        ? fileActions.getFileActions({ path: child.path, name: child.title }, extraActions)
        : [
            { label: 'Open', color: 'var(--accent)', onClick: () => onActionClick && onActionClick(child) },
            ...(child.title ? [{ label: 'Copy File Name', onClick: () => navigator.clipboard.writeText(child.title) }] : []),
            ...extraActions
        ];
