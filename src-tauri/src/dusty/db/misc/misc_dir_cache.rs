...
pub fn create_misc_dir_cache_table(db: &Connection) -> Result<()> {
    ...
    .map_err(|err| {
        DustyError::db(
            "create_misc_dir_cache_table",
            Some("misc_dir_cache".to_string()),
            err,
        )
    })?;
    Ok(())
...
pub fn get_misc_dir_cache(db: &Connection, misc_type: &str) -> Result<Vec<MiscDir>> {
    ...
    let mut stmt = db
        ...
        .map_err(|err| {
            DustyError::db(
                "prepare_get_misc_dir_cache",
                Some("misc_dir_cache".to_string()),
                err,
            )
        })?;

    let mut rows = stmt.query(params![id, misc_type]).map_err(|err| {
        DustyError::db(
            "query_get_misc_dir_cache",
            Some("misc_dir_cache".to_string()),
            err,
        )
    })?;
    ...
    if let Ok(Some(row)) = rows.next() {
        let json_data: String = row.get(0).map_err(|err| {
            DustyError::db(
                "read_row_misc_dir_cache",
                Some("misc_dir_cache".to_string()),
                err,
            )
        })?;
...
pub fn reset_misc_dir_cache(conn: &Connection, misc_type: &str) -> Result<()> {
    let id = format!("root_{}_tree", misc_type);
    conn.execute(
        "DELETE FROM misc_dir_cache WHERE id = ?1 AND misc_type = ?2",
        params![id, misc_type],
    )
    .map_err(|err| {
        DustyError::db(
            "reset_misc_dir_cache",
            Some("misc_dir_cache".to_string()),
            err,
        )
    })?;
