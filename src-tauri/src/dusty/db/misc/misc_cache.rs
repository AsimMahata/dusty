...
pub fn create_misc_cache_table(db: &Connection) -> Result<()> {
    ...
    .map_err(|err| {
        DustyError::db(
            "create_misc_cache_table",
            Some("misc_cache".to_string()),
            err,
        )
    })?;
    Ok(())
...
pub fn add_or_update_misc_cache(db: &Connection, file: &FileInfo, misc_type: &str) -> Result<()> {
    ...
    let data =
        serde_json::to_string(file).map_err(|e| DustyError::serde("serialize_file_info", e))?;
...
pub fn get_misc_cache(db: &Connection, misc_type: &str) -> Result<Vec<FileInfo>> {
    ...
    let file_iter = stmt
        ...
        .map_err(|err| {
            DustyError::db("query_get_misc_cache", Some("misc_cache".to_string()), err)
        })?;
...
pub fn reset_misc_cache(conn: &Connection, misc_type: &str) -> Result<()> {
    conn.execute(
        "DELETE FROM misc_cache WHERE misc_type = ?1",
        params![misc_type],
    )
