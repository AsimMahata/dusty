...
pub fn create_empty_dir_cache_table(db: &Connection) -> Result<()> {
    ...
    .map_err(|err| {
        DustyError::db(
            "create_empty_dir_cache_table",
            Some("empty_dir_cache".to_string()),
            err,
        )
    })?;
    Ok(())
...
pub fn add_or_update_empty_dir_cache(db: &Connection, file: &FileInfo) -> Result<()> {
    ...
    let data =
        serde_json::to_string(file).map_err(|e| DustyError::serde("serialize_empty_dir", e))?;
