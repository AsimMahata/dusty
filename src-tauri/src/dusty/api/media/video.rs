...
pub fn scan_video_using_cache(
    ...
    if cache {
        if let Some(cached_videos) = get_cached_videos_from_media_cache_db(&db, path) {
    ...
    add_videos_in_media_cache_table(&db, path, &file_info_list)
        .map_err(|err| {
            logger::warning!("ADD_VIDEOS_IN_CACHE_ERROR", err);
        })
        .ok();
...
pub fn add_videos_in_media_cache_table(
    ...
    let id = get_sha256_id(path.clone(), "flat_video".to_string());
    let data: String = serde_json::to_string(&videos).unwrap_or_default();
    db.execute(
