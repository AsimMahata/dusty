...
#[tauri::command]
pub fn update_anime_info_in_mal_cache(state: tauri::State<AppState>, id: i32, data: String) -> Result<(), String> {
    let db = state.db.lock().unwrap();
...
#[tauri::command]
pub fn add_anime_info_to_mal_cache(state: tauri::State<AppState>, id: i32, data: String) -> Result<(), String> {
    let db = state.db.lock().unwrap();
